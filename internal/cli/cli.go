// Package cli implements the shu command surface.
package cli
...
Usage:
  ...
  shu add <skill>[@constraint] --from <source>
  ...
  shu status [--json] [--offline]   Drift report; exit 1 if anything is not aligned
             [--fleet]                aggregate across every repo in fleet.json
  shu diff <skill> [--latest]       Diff working copy vs ancestor (--latest: ancestor vs upstream)
  shu update [<skill>...]           Pull upstream changes: fast-forward or 3-way merge that
                                      preserves local edits; exit 1 on conflicts
             [--ai-merge]             propose conflict resolutions via $SHU_AI_MERGE_CMD (never auto-applied)
  shu verify [--json]               Fail unless working tree and snapshots match skills.lock
  shu project [<skill>...]          (Re)render skills into their configured surfaces
  shu eval <skill> [--scaffold]     Run the skill's eval suite via a promptfoo-compatible runner
  shu harvest <skill> [--push]      Package local drift as an attributed branch against upstream
  shu propagate [--push]            Fleet fan-out: update every fleet.json repo, gate on evals,
                                      prepare (or push) a PR branch per repo
  shu reconcile [--json]            Cluster shu/harvest/* branches; flag overlapping learnings
  shu scan [<skill>...] [--json]    Flag prompt-injection patterns and risky payloads; exit 1 on high
  shu audit [<skill>...] [--json]   Content-level change log: who changed which skill, when
...
func fail(err error) int {
	fmt.Fprintln(os.Stderr, "shu:", err)
	return 1
}
